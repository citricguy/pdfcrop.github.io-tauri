...
import {
    isDesktopApp,
    openPdfWithPlatformDialog,
    savePdfWithPlatformDialog
} from './platform-file-io';
...
/**
 * Load PDF data into the application state and viewer
 */
async function loadPdfData(pdfData: Uint8Array, filename: string): Promise<void> {
...
/**
 * Open a PDF using the desktop file picker when running in Tauri
 */
async function handleDesktopOpen(): Promise<void> {
    const selectedFile = await openPdfWithPlatformDialog();
    if (!selectedFile) {
        return;
    }

    await loadPdfData(selectedFile.data, selectedFile.name);
}
...
async function handleCrop(): Promise<void> {
    ...
    try {
        ...
        const saved = await savePdfWithPlatformDialog(croppedPDF, outputFilename);
        if (!saved) {
